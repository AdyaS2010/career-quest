  ...
  useEffect(() => {
    ...
    const loop = (t: number) => {
      ...
      if (key !== nearKeyRef.current) {
        ...
        setNearLabel(near === 'npc' ? 'Talk to Mayor Questopher' : near ? (near.slug === 'home' ? 'Enter Home' : `Enter ${near.name}`) : null);
      }
...
function drawBanner(ctx: CanvasRenderingContext2D, x: number, y: number, color: string, t: number, lightsOn = false) {
  ...
  // 2. Sleek dark iron pole
  ctx.fillStyle = '#374151'; ctx.fillRect(-1, -36, 2, 37);                 // pole
  
  // 3. Post-top lantern fixture
  // Base
